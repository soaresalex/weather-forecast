...
function showWeatherDetails(jsonObj) {
	...
	myH1.textContent = jsonObj.list[2].name + ", " + jsonObj.list[2].sys.country;
	document.getElementById("city-name").appendChild(myH1);

	// Show current weather condition
	var myImg = document.createElement("img");
	if (jsonObj.list[2].weather[0].icon === "01d") {
		myImg.setAttribute("src", "../img/clear-sky-day.png");
	} else if (jsonObj.list[2].weather[0].icon === "01n") {
		myImg.setAttribute("src", "../img/clear-sky-night.png");
	} else if (jsonObj.list[2].weather[0].icon === "02d") {
		myImg.setAttribute("src", "../img/partly-cloudy-day.png");
	} else if (jsonObj.list[2].weather[0].icon === "02n") {
		myImg.setAttribute("src", "../img/partly-cloudy-night.png");
	} else if (jsonObj.list[2].weather[0].icon === "03d" || jsonObj.list[2].weather[0].icon === "03n" || jsonObj.list[2].weather[0].icon === "04d" || jsonObj.list[2].weather[0].icon === "04n") {
		myImg.setAttribute("src", "../img/cloudy.png");
	} else if (jsonObj.list[2].weather[0].icon === "09d" || jsonObj.list[2].weather[0].icon === "09n") {
		myImg.setAttribute("src", "../img/shower-rain.png");
	} else if (jsonObj.list[2].weather[0].icon === "10d") {
		myImg.setAttribute("src", "../img/rain-day.png");
	} else if (jsonObj.list[2].weather[0].icon === "10n") {
		myImg.setAttribute("src", "../img/rain-night.png");
	} else if (jsonObj.list[2].weather[0].icon === "11d") {
		myImg.setAttribute("src", "../img/thunderstorm-day.png");
	} else if (jsonObj.list[2].weather[0].icon === "11n") {
		myImg.setAttribute("src", "../img/thunderstorm-night.png");
	} else if (jsonObj.list[2].weather[0].icon === "13d") {
		myImg.setAttribute("src", "../img/snow-day.png");
	} else if (jsonObj.list[2].weather[0].icon === "13n") {
		myImg.setAttribute("src", "../img/snow-night.png");
	} else if (jsonObj.list[2].weather[0].icon === "50d") {
		myImg.setAttribute("src", "../img/mist-day.png");
	} else if (jsonObj.list[2].weather[0].icon === "50n") {
	...
	myH2.textContent = Math.round(jsonObj.list[2].main.temp) + "\u00B0F";
	document.getElementById("temperature").appendChild(myH2);

	// Show the weather condition in text
	var description = document.createElement('p');
	description.textContent = jsonObj.list[2].weather[0].description;
	...
	tempMinMax.textContent = "H " + Math.round(jsonObj.list[2].main.temp_min) + "\u00B0F | L " + Math.round(jsonObj.list[2].main.temp_max) + "\u00B0F";
	document.getElementById("temperature").appendChild(tempMinMax);

	// Show wind speed
	var wind = document.createElement('p');
	wind.innerHTML = "<strong>Wind Speed</strong>" + jsonObj.list[2].wind.speed;
	document.getElementById("more-details").appendChild(wind);

	// Show humidity
	var humidity = document.createElement('p');
	humidity.innerHTML = "<strong>Humidity</strong>" + jsonObj.list[2].main.humidity + "%";
	document.getElementById("more-details").appendChild(humidity);

	// Show pressure
	var pressure = document.createElement('p');
	pressure.innerHTML = "<strong>Pressure</strong>" + jsonObj.list[2].main.pressure + " hpa";
	document.getElementById("more-details").appendChild(pressure);

	// Show geo coordinates
	var coordinates = document.createElement('p');
	coordinates.innerHTML = "<strong>Geo coords</strong>" + "<u>[" + jsonObj.list[2].coord.lat + ", " + jsonObj.list[2].coord.lon + "]</u>";
	...
	$(document).ready(function () {
		$("#coord").click(function () {
			window.open("https://www.google.com/maps/search/?api=1&query=" + jsonObj.list[2].coord.lat + "," + jsonObj.list[2].coord.lon);
		});
	});
	...
	var d = new Date();
	d.setTime(jsonObj.list[2].sys.sunrise);
	var h = d.getHours();
	var m = d.getMinutes();
	...
	var x = new Date();
	d.setTime(jsonObj.list[2].sys.sunset);
	var hh = x.getHours();
	var mm = x.getMinutes();
